...
def load_wav(full_path, sample_rate=16000):
...
def get_dataset_filelist():
    with open("datasets/training.txt", 'r', encoding='utf-8') as fi:
        training_files = [x.replace('\n', '').replace('\r', '') for x in fi.readlines() if len(x) > 0]

    with open("datasets/validation.txt", 'r', encoding='utf-8') as fi:
        validation_files = [x.replace('\n', '').replace('\r', '') for x in fi.readlines() if len(x) > 0]
